...
RWS-weakestPre : (m : RWS Ev Wr St A) → RWS-PredTrans Ev Wr St A
  ...
  preconditions in this way has a number of benefits, described below.

  Below we overview how we compute weakest preconditions, and then how we use them to reason about handlers.
