...
*** Type classes for branching operations
    ...
    Peer handler code written the =LBFT= monad uses
    ...
    constructors for the =RWS= datatype.  However, it is desirable to keep the code
    ...
    ([[file:~/Projects/olabs_blockchain/bft-consensus-agda/src/Dijkstra/Syntax.agda][file:../src/Dijkstra/Syntax.agda]]), which provides a monad and a method (called =ifD‖_)= to
    ...
    =RWS-MonadIfD= ([[file:~/Projects/olabs_blockchain/bft-consensus-agda/src/Dijkstra/RWS/Syntax.agda][file:../src/Dijkstra/RWS/Syntax.agda]]; recall that =LBFT= is just a
