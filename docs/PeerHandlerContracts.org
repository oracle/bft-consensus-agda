...
** Reasoning about programs in the =Either= monad
  ...
  machinery is slightly differently.  In particular, because Agda
  ...
  (=EitherD-run=), a definition of weakest precondition for code in =EitherD=, etc., similar to =RWS=.

*** Connecting monadic code to =EitherD=

    TODO: the rest of this section is historical and has all the =VariantFor= stuff, which seems to
    entail too much overhead for the benefit it provides.  I suspect that this can be streamlined by
    writing all code that is in the =Either= monad in Haskell in =EitherD=, and when we need to pattern
    match on the result of such code, use =EitherD-run= to ensure that only =Left= and =Right= cases are
    considered.  I am not sure about this, and I would like to get some feedback and discussion.  I
    also think there are some useful lessons to be learned here, so it could be worth writing up the
    various approaches and their pros and cons, even if we decide to streamline all of this into one
    approach.  In any case, we have proofs that use various approaches, so we should keep documenting
    all of the approaches unless and until we do so.
