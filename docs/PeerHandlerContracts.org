...
module ensureRoundAndSyncUpMSpec
  ...
  module _ (pre : RoundManager) where
    ...
    record Contract (r : Either ErrLog Bool) (post : RoundManager) (outs : List Output) : Set where
     ...
     We define the property we want to prove and call it =Contract= --- from outside the module,
     ...
     and =Output=​s generated.

     As =ensureRoundAndSyncUpM= is defined as =ensureRoundAndSyncUpM.step₀=, the proof is
     for code that first extracts a =Round= from the pre-state (via =use=), binds the result
     ...
     =pre= satisfies the weakest precondition of |Contract| for the =then= branch.  The second underscore is an anonymous proof that the
     conditional evaluated to true (which is not needed for this proof).
    ...
    proj₁ (contract' ._ refl) _ =
     ...
     The first four fields of |Contract| are trivially satisfied because =step₀= (and therefore =ensureRoundAndSyncUpM=)
     ...
     The proof that the empty list contains no =Output= that contains a =QC= is vacuous: =[]=.
