...
* Structure of peer handler proofs
  ...
  the =RWS= --- /reader, writer, state/ --- and =Either= monads.

** RWS monad

  In Agda, the ~RWS~ monad is defined as a special-purpose datatype
  (in [[file:../src/Haskell/Modules/RWS.agda]]).
  This enables proofs to inspect the AST of peer handler code written in the ~RWS~ monad directly.

  This is primarily used to compute, for any
...
*** Breaking the peer handler down into smaller "steps"
    ...
    Let's look at =ensureRoundAndSyncUpM= ([[file:~/Projects/olabs_blockchain/bft-consensus-agda/src/LibraBFT/Impl/Consensus/RoundManager.agda][file:../src/LibraBFT/Impl/Consensus/RoundManager.agda]]) as an example.
...
module ensureRoundAndSyncUpMSpec
  ...
  module _ (pre : RoundManager) where
    ...
    - =SU.noVoteOuts= tells us there are no vote messages in =outs=, but our obligation is to show there
      are no vote messages in =outs ++ []=.  The proof that the empty list contains no =Output= that
      contains a =QC= is vacuous: =[]=.
