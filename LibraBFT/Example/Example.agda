...
module LibraBFT.Example.Example2 (#peers : ℕ) where

 open import LibraBFT.Global.SystemModel2 #peers
 ...
 record PeerState : Set where
   constructor mkState
   field
     :myId         : Peer
     :maxSeen      : ℕ
     :newValSender : Maybe Peer
 open PeerState
 unquoteDecl myId   maxSeen   newValSender = mkLens (quote PeerState)
            (myId ∷ maxSeen ∷ newValSender ∷ [])

 PeerState₀ : Peer → PeerState
 PeerState₀ p = mkState p 0 nothing 

 -- The handler will be defined inside RWST; just like
 -- we would do in Libra
 open RWST-do

 -- I renamed Output to Output;
 ...
 ...| just 1stSender = (confirmedAdvance (msg ^∙ val))
                     , announce (msg ^∙ val)   
                     ∷ broadcast (suc (msg ^∙ val)) 
                     ∷ []

 handle : Message → RWST Unit Output PeerState Unit
 handle msg = do  -- TODO: Check signature
   st ← get
   let res , outs = pureHandler msg st
   case res of
     λ { noChange             → pure unit
       ...
       ; (confirmedAdvance v) → do
           ...
           -- VCM: Oh... that's why there was
           -- a timestamp put on the sent action. I removed that and
           -- made a broadcast instead.
           maxSeen ∙= v
           newValSender ∙= nothing
       }
   tell outs -- TODO: none of the Dec proofs are used here; can this be simplified?

 processOutput : PeerState → Output → List (Peer × Message)
 processOutput s (announce _) 
   = []
 processOutput s (broadcast n) 
   = flip List-map (fins #peers) (λ pid 
      → pid , mkMessage (s ^∙ myId) n nothing)  -- TODO: add signatures?

 -- The example still uses the same paradigm of Pure handler, which
 -- produces outputs (actions) and are further processed; but the system
 -- model just cares about the step function:
 stepPeer : PeerState → Message → (List (Peer × Message) × PeerState)
 stepPeer st m = let res , st' , outs = RWST-run (handle m) unit st
                  in concat (List-map (processOutput st') outs) , st'

 -- Instantiate the SystemModel for our Example system
 open WithParms Message sig-Message PeerState PeerState₀ stepPeer 
   
 recordedValueWasSent : SysState → Set
 recordedValueWasSent st = ∀ {curMax}
                         → (sender p : Peer)
                         → peerState st p ^∙ newValSender ≡ just sender
                         → peerState st p ^∙ maxSeen ≡ curMax
                         → Σ (Peer × Message) (λ { (recip , msg)
                             → WithVerSig msg × (recip , msg) ∈SM sentMessages st
                                              × msg ^∙ author ≡ sender
                                              × msg ^∙ val    ≡ suc curMax })

 newValSender₀≡nothing : (p : Peer) → (PeerState₀ p) ^∙ newValSender ≡ nothing
 newValSender₀≡nothing p = refl

 open import Data.Vec.Properties
   using (lookup∘tabulate)

 proof : Invariant recordedValueWasSent
 proof {s} Star.ε       sender p isSender hyp 
   rewrite newValSender₀≡nothing p 
         | lookup∘tabulate PeerState₀ p 
         = ⊥-elim (maybe-⊥ isSender refl)
 proof {s} (e@(peerStep p' m m∈st) Star.◅ r) sender p isSender hyp 
   with p ≟Fin p'
 ...| no p≢p' rewrite peerState-stable e p p≢p' 
    = {!!}
 ...| yes refl = {!!}
...
{-
 ...
 (gotFirstAdvance p1) ≟HR (gotFirstAdvance p2) with p1 ≟Fin p2
 ...| yes refl = yes refl
...
{-
 ...
 exampleOutputsToSends : State → Output → List (Peer × Message)
 exampleOutputsToSends s (announce _) = []
 exampleOutputsToSends s (send n peer) =  (peer , (mkMessage (s ^∙ myId) n nothing)) ∷ []  -- TODO: sign message
 ...
 recordedValueWasSent st = ∀ {pSt curMax}
                           ...
                           → pSt ^∙ maxSeen ≡ curMax
                           → Σ Peer λ recip → Σ Message (λ msg → WithVerSig msg
 ...
 ...| pSt≡ppost | ppost≡
    ...
    with Maybe-≡-dec _≟-Peer_ (:newValSender ppre) (just sender) | curMax ≟ :maxSeen ppre
 ...| yes refl | yes refl
