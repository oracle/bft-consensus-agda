...
...| handleProposalSpec.mkContract _ _ (Voting.mkVoteAttemptCorrectWithEpochReq (Left (_ , voteUnsent)) sdEpoch≡?) =
...
sameERasLV⇒sameId{pid = .pid“}{pid'}{pk} (step-s{pre = pre} preach step@(step-peer{pid“} sp@(step-honest sps@(step-msg{_ , P pm} pm∈pool ini)))){v}{v'} hpk ≡pidLV pcsfpk ._ _ sig' ¬gen ≡epoch ≡round
   | Left (m∈outs , pcsfpk' , ¬msb4) | refl | vote∈vm = ret
  where
  -- Definitions
...
sameERasLV⇒sameId{.pid“}{pid'}{pk} (step-s{pre = pre} preach step@(step-peer{pid“} sp@(step-honest sps@(step-msg{_ , P pm} m∈pool ini)))){v}{v'} hpk ≡pidLV pcsfpk v'⊂m' m'∈pool sig' ¬gen ≡epoch ≡round
   | Right mws∈pool | yes refl | vote∈vm =
  trans hyp (cong (_^∙ vdProposed ∙ biId) ≡voteData)
  where
  -- Definitions
  hpPre = peerStates pre pid“
  ...
  ...| Voting.mkVoteAttemptCorrectWithEpochReq (Left (_ , Voting.mkVoteUnsentCorrect noVoteMsgOuts nvg⊎vgusc)) sdEpoch≡?
    with nvg⊎vgusc
  ...| Left (StateTransProps.mkVoteNotGenerated lv≡ lvr≤) = OldVote.hyp lv≡
  ...| Right (Voting.mkVoteGeneratedUnsavedCorrect vote (Voting.mkVoteGeneratedCorrect (StateTransProps.mkVoteGenerated lv≡v voteSrc) blockTriggered))
    with voteSrc
  ...| Left (StateTransProps.mkVoteOldGenerated lvr≡ lv≡) = OldVote.hyp lv≡
  ...| Right (StateTransProps.mkVoteNewGenerated lvr< lvr≡) =
    ⊥-elim (<⇒≢ (NewVote.rv'<rv vote lv≡v lvr< lvr≡ sdEpoch≡? blockTriggered) (sym ≡round))
  hyp | Voting.mkVoteAttemptCorrectWithEpochReq (Right (Voting.mkVoteSentCorrect vm pid voteMsgOuts (Voting.mkVoteGeneratedCorrect (StateTransProps.mkVoteGenerated lv≡v voteSrc) blockTriggered))) sdEpoch≡?
    with voteSrc
  ...| Left (StateTransProps.mkVoteOldGenerated lvr≡ lv≡) = OldVote.hyp lv≡
  ...| Right (StateTransProps.mkVoteNewGenerated lvr< lvr≡) =
...
votesOnce₁{pid = pid}{pid'}{pk = pk}{pre = pre} preach sps@(step-msg{sndr , V vm} m∈pool ini){v}{m}{v'}{m'} hpk v⊂m m∈outs sig ¬gen ¬msb vspk v'⊂m' m'∈pool sig' ¬gen' eid≡
  with v⊂m
... | vote∈qc vs∈qc v≈rbld qc∈m = ⊥-elim (¬msb TODO)
  where
