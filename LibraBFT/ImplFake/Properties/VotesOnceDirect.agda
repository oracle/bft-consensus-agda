...
module LibraBFT.ImplFake.Properties.VotesOnceDirect (𝓔 : EpochConfig) where
  ...
  ...| vote∈qc vs∈qc v≈rbld (inSI si∈m qc∈si) rewrite cong _vSignature v≈rbld
       = let qc∈rm = VoteMsgQCsFromRoundManager r step pkH v⊂m (here refl) (obm-dangerous-magic' "see Handle.Properties")
         in ⊥-elim (vnew (qcVotesSentB4 r pinit qc∈rm vs∈qc ¬bootstrap))
  ...
  MsgWithSig⇒ValidSenderInitialised {st} {v} (step-s r step@(step-peer (step-honest {pid} stP))) pkH sig ¬bootstrap msv
     with msgSameSig msv
  ...| refl
     with newMsg⊎msgSentB4 r stP pkH (msgSigned msv) ¬bootstrap (msg⊆ msv) (msg∈pool msv)
  ...
  MsgWithSig⇒ValidSenderInitialised {st} {v} (step-s r step@(step-peer (step-honest stP))) pkH sig ¬bootstrap msv
     | refl
     | inj₂ msb4
     with MsgWithSig⇒ValidSenderInitialised {v = v} r pkH sig ¬bootstrap msb4
  ...| pid , initP , pcsPre = pid ,
                              peersRemainInitialized step initP ,
                              PeerCanSignForPK-stable r step pcsPre
  MsgWithSig⇒ValidSenderInitialised {st} {v} (step-s r step@(step-peer cheat@(step-cheat x))) pkH sig ¬bootstrap msv
     with msgSameSig msv
  ...| refl
     with ¬cheatForgeNew cheat refl unit pkH msv ¬bootstrap
  ...| msb4
     with MsgWithSig⇒ValidSenderInitialised {v = v} r pkH sig ¬bootstrap msb4
  ...| pid , initP , pcsPre = pid ,
                              peersRemainInitialized step initP ,
  ...
  peerCanSign-Msb4 r step (mkPCS4PK 𝓔₁ (inBootstrapInfo refl) (mkPCS4PKin𝓔 𝓔id≡₁ mbr₁ nid≡₁ pk≡₁)) pkH sig msv
  ...
  msg∈pool⇒initd {pid'} {st = st} step@(step-s r (step-peer {pid} (step-honest stPeer))) pcs pkH sig ¬bootstrap msv
     with msgSameSig msv
  ...| refl
     with newMsg⊎msgSentB4 r stPeer pkH (msgSigned msv) ¬bootstrap (msg⊆ msv) (msg∈pool msv)
  ...| inj₁ (m∈outs , pcsN , newV)
  ...
  msg∈pool⇒initd {pid'} (step-s r step@(step-peer {pid} (step-honest stPeer))) pcs pkH sig ¬bootstrap msv
     | refl
     | inj₂ msb4
     with pid ≟ pid'
  ...| yes refl = refl
  ...| no  pid≢ = let pcsmsb4 = peerCanSign-Msb4 r step pcs pkH sig msb4
                  in msg∈pool⇒initd r pcsmsb4 pkH sig ¬bootstrap msb4
  msg∈pool⇒initd {pid'} (step-s r step@(step-peer {pid} cheat@(step-cheat c))) pcs pkH sig ¬bootstrap msv
     with msgSameSig msv
  ...| refl
     with ¬cheatForgeNew cheat refl unit pkH msv ¬bootstrap
  ...| msb4
       = let pcsmsb4 = peerCanSign-Msb4 r step pcs pkH sig msb4
             initPre = msg∈pool⇒initd r pcsmsb4  pkH sig ¬bootstrap msb4
  ...
  noEpochChange r (step-init uni) pcs pkH sig ∉bootstrap msv eid≡
    = ⊥-elim (uninitd≢initd (trans (sym uni) (msg∈pool⇒initd r pcs pkH sig ∉bootstrap msv)))
  noEpochChange r sm@(step-msg _ ini) pcs pkH sig ∉bootstrap msv eid≡
  ...
  oldVoteRound≤lvr {pid'} (step-s r step@(step-peer {pid = pid} cheat@(step-cheat c)))
                   pkH sig ¬bootstrap msv vspk eid≡
     with ¬cheatForgeNew cheat refl unit pkH msv (¬subst ¬bootstrap (msgSameSig msv))
  ...| msb4 rewrite cheatStepDNMPeerStates₁ {pid = pid} {pid' = pid'} cheat unit
       = let pcsmsb4 = peerCanSign-Msb4 r step vspk pkH sig msb4
         in oldVoteRound≤lvr r pkH sig ¬bootstrap msb4 pcsmsb4 eid≡
  ...
  ...| pcsmsb4
     with pid ≟ pid'
  ...| no  pid≢ = oldVoteRound≤lvr r  pkH sig ¬bootstrap msb4 pcsmsb4 eid≡
  ...| yes refl = let  initP = msg∈pool⇒initd r pcsmsb4 pkH sig ¬bootstrap msb4
                       ep≡   = noEpochChange r stPeer pcsmsb4 pkH sig ¬bootstrap msb4 eid≡
                       lvr≤  = lastVoteRound-mono r refl stPeer initP (trans ep≡ (sym eid≡))
                   in ≤-trans (oldVoteRound≤lvr r pkH sig ¬bootstrap msb4 pcsmsb4 ep≡) lvr≤
  ...
  votesOnce₁ {pid' = pid'} r stMsg@(step-msg {_ , P m} m∈pool psI) {v' = v'} {m' = m'}
             pkH v⊂m (here refl) sv ¬bootstrap ¬msb vspk v'⊂m' m'∈pool sv' ¬bootstrap' eid≡
     with v⊂m
  ...| vote∈qc vs∈qc v≈rbld (inSI si∈m qc∈si) rewrite cong _vSignature v≈rbld
     = let qc∈rm = VoteMsgQCsFromRoundManager r stMsg pkH v⊂m (here refl) (obm-dangerous-magic' "see Handle.Properties")
       in ⊥-elim (¬msb (qcVotesSentB4 r psI qc∈rm vs∈qc ¬bootstrap))
  ...| vote∈vm
     with ⊎-elimʳ ¬msb (impl-sps-avp r pkH stMsg (here refl) v⊂m sv ¬bootstrap)
  ...| (vspkv , _) =
                 let m'mwsb = mkMsgWithSig∈ m' v' v'⊂m' pid' m'∈pool sv' refl
                     ...
                     rv'<rv = oldVoteRound≤lvr r pkH sv' ¬bootstrap' m'mwsb vspre' eid≡
                 in inj₁ (s≤s rv'<rv)

  votesOnce₂ : VO.ImplObligation₂ FakeInitAndHandlers 𝓔
  votesOnce₂ {pk = pk} {st} r stMsg@(step-msg {_ , P m} m∈pool psI) pkH v⊂m m∈outs sig ¬bootstrap vnew
             vpk v'⊂m' m'∈outs sig' ¬bootstrap' v'new vpk' es≡ rnds≡
     with m∈outs | m'∈outs
  ...| here refl | here refl
     with v⊂m                          | v'⊂m'
  ...
  ...| vote∈vm                         | vote∈qc vs∈qc' v≈rbld' (inSI si∈m' qc∈si)
       ...
       = let qc∈rm' = VoteMsgQCsFromRoundManager r stMsg pkH v'⊂m' (here refl) (obm-dangerous-magic' "see Handle.Properties")
         in ⊥-elim (v'new (qcVotesSentB4 r psI qc∈rm' vs∈qc' ¬bootstrap'))
  ...| vote∈qc vs∈qc v≈rbld (inSI si∈m qc∈si) | _
       rewrite cong _vSignature v≈rbld
       = let qc∈rm = VoteMsgQCsFromRoundManager r stMsg pkH v⊂m (here refl) (obm-dangerous-magic' "see Handle.Properties")
         in ⊥-elim (vnew (qcVotesSentB4 r psI qc∈rm vs∈qc ¬bootstrap))
